...
namespace Cadastro.Controllers
{
    ...
    public class CupomController : Controller
    {
        ...
        public async Task<IActionResult> CadastrarCupom([FromBody] CupomDto cupomDto)
        {
            ...
            if (_cupomServico.EhQuantidadeMaximaProdutos(cupomDto.produtos.Count))
            {
                ModelState.AddModelError("QuantidadeMaxima", "Quantidade máxima de produtos por cupom é 5.");
            }

            if (_cupomServico.EhQuantidadeMinimaProdutos(cupomDto.produtos.Count))
            {
                ModelState.AddModelError("QuantidadeMinima", "Quantidade mínima de produtos por cupom é 1.");
            }

            if (!_cupomServico.EhNumeroCupomFiscalValido(cupomDto.numeroCupom))
            {
                ModelState.AddModelError("NumeroCupomFiscal", "Número do cupom fiscal deve ter 8 dígitos.");
            }

            if (!await _cupomServico.EhNumeroCupomFiscalUnico(cupomDto.numeroCupom))
            {
                ModelState.AddModelError("NumeroCupomFiscalUnico", "Número do cupom fiscal já cadastrado.");
            }

            if (!_cupomServico.ehCnpjValido(cupomDto.cnpj))
            {
                ModelState.AddModelError("CnpjEstabelecimento", "CNPJ do estabelecimento inválido.");
            }
            ...
            if (!_cupomServico.EhValorValido(cupomDto.ValorTotal))
            {
            ...
            try
            {
                ...
                _contexto.EmailLogs.Add(new EmailLog
                {
                    ...
                    Email = usuario.Email,
                    Status = "Enviado",
                    Mensagem = "Cadastro de cupom realizado realizado com sucesso"
                });
