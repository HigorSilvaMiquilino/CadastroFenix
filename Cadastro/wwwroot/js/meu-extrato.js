...
(async () => {
    try {
        const response = await fetchWithAuth("https://localhost:7011/api/v1/Cupom/obtertodos", {
            method: "GET",
            credentials: "include"
        });
        ...
        if (response.ok) {
            ...
            const cupons = result.Metadata.cupons || []; 

            if (cupons.length === 0) {
                document.querySelector("table tbody").innerHTML = `
                    <tr>
                        <td colspan="5" style="text-align: center;">Nenhum cupom encontrado.</td>
                    </tr>`;
                return;
            }
            ...
            let html = "";
            cupons.forEach(data => {
                const cnpj = data.CnpjEstabelecimento.replace(
                    /^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/,
                    "$1.$2.$3/$4-$5"
                );
